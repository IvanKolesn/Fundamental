...
Tickerb<-c("VNET","ACIA","CRTO","ALRM","GDS","GSUM","INOV","INSE","JMU","ENPH","PSDO","QLYS","RESN","TRUE","VERI","YNDX","ZDGE",
...
Final<-matrix(NA,nrow = 2,ncol = length(Tickers))
...
#FCFE<-NI+D&A-Capex-Delta_WC
Delta_Wc<-c(0,diff(Data$workingcapital,1))
FCFE<-Data$netinc+Data$depamor-Data$capex-Delta_Wc

#Forecast
Forecast<-forecast(ets(FCFE),h=4)
Forecast<-Forecast$mean %>% as.vector()
#Check
data<-matrix(NA,nrow=(length(FCFE)+length(Forecast)),ncol=3)
data[,1]<-c(FCFE,Forecast)
data[,2]<-c(rep(0, length.out = length(FCFE)), rep(1, length.out = length(Forecast)))
data[,3]<-seq(1,nrow(data),1)
data<-as.data.frame(data)
colnames(data)<-c("FCFE","Shift","Trend")
lm(FCFE~Trend+Shift,data=data) %>% summary()
...
Price_FCFE<-sum(V$Value)/tail(Data$sharesbas,1)

Final[1,i]<-Price_FCFE
Final[2,i]<-tail(Data$price,1)
}
